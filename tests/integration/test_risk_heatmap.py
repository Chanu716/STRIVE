...
def build_graph() -> nx.MultiDiGraph:
    ...
    graph.add_edge(2, 3, key=0, length=100.0, speed_kph=50.0, highway="secondary", geometry=LineString([(1.0, 0.0), (2.0, 0.0)]))
    return graph
...
def patch_dependencies(monkeypatch):
    ...
    explanation = ExplanationResult(
        risk_score=72,
        probability=0.72,
        shap_values={"hour_of_day": 0.1, "day_of_week": 0.1, "month": 0.1, "night_indicator": 0.5, "road_class": 0.2, "speed_limit_kmh": 0.1, "precipitation_mm": 0.8, "visibility_km": -0.1, "wind_speed_ms": 0.0, "temperature_c": 0.0, "rain_on_congestion": 0.3, "historical_accident_rate": 0.4},
        expected_value=0.25,
    )
...
async def test_heatmap_endpoint(monkeypatch):
    patch_dependencies(monkeypatch)
    transport = ASGITransport(app=main.app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/v1/risk/heatmap", params={"bbox": "-1,-1,3,1"})
