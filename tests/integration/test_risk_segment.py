...
def patch_dependencies(monkeypatch):
    ...
    explanation = ExplanationResult(
        ...
        shap_values={
            ...
            "wind_speed_ms": 0.0,
            "temperature_c": 0.0,
            "rain_on_congestion": 0.3,
            "historical_accident_rate": 0.4,
        },
        expected_value=0.25,
    )
...
async def test_segment_risk_endpoint(monkeypatch):
    patch_dependencies(monkeypatch)
    transport = ASGITransport(app=main.app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/v1/risk/segment", params={"lat": 0.1, "lon": 0.1, "datetime": "2024-06-15T22:00:00"})

    assert response.status_code == 200
    payload = response.json()
    assert payload["risk_score"] == 72
    assert payload["risk_level"] == "HIGH"
    assert len(payload["shap_top_factors"]) == 4
    assert payload["segment_id"] == "1_2"
